...
/*
export async function showGame(container: HTMLElement, mode: GameMode = "duo"): Promise<() => void> {
    container.innerHTML = "Looking for match...";
    ...
    function keyDown(e: KeyboardEvent) { keys.add(e.key); }
    ...
    ws.onmessage = (event) => {
        const msg = JSON.parse(event.data);
        if (msg.type === "identify") myPaddleIndex = msg.index;
        if (msg.type === "state") {
            const s = msg.state;
            state.ballX = s.ball.x;
            state.ballY = s.ball.y;
    ...
    const intervalId = setInterval(() => {
        ...
        let value = null;
        if (myPaddleIndex === 1) {
            if (keys.has("w")) value = state.player1Y - PADDLE_SPEED;
            if (keys.has("s")) value = state.player1Y + PADDLE_SPEED;
            if (value !== null) ws.send(JSON.stringify({ type: "paddle", axis: "y", value }));
        }
        if (myPaddleIndex === 2) {
            if (keys.has("ArrowUp")) value = state.player2Y - PADDLE_SPEED;
            if (keys.has("ArrowDown")) value = state.player2Y + PADDLE_SPEED;
            if (value !== null) ws.send(JSON.stringify({ type: "paddle", axis: "y", value }));
        }
        if (myPaddleIndex === 3 && mode === "quad") {
            if (keys.has("a")) value = (state.player3X ?? 0) - PADDLE_SPEED;
            if (keys.has("d")) value = (state.player3X ?? 0) + PADDLE_SPEED;
            if (value !== null) ws.send(JSON.stringify({ type: "paddle", axis: "x", value }));
        }
        if (myPaddleIndex === 4 && mode === "quad") {
            if (keys.has("j")) value = (state.player4X ?? 0) - PADDLE_SPEED;
            if (keys.has("l")) value = (state.player4X ?? 0) + PADDLE_SPEED;
            if (value !== null) ws.send(JSON.stringify({ type: "paddle", axis: "x", value }));
        }
    }, 1000 / 60);
